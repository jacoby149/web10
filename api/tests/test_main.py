"""Tests for authentication & authorization logic in main.py.
...
class TestPasswordHashing:
    ...
    def test_verify_correct_password(self, mocker):
        mocker.patch.object(main.pwd_context, "verify", return_value=True)
        assert main.verify_password("secret123", "fake_hash") is True

    def test_verify_wrong_password(self, mocker):
        mocker.patch.object(main.pwd_context, "verify", return_value=False)
        assert main.verify_password("wrong", "fake_hash") is False

    def test_hash_delegates_to_context(self, mocker):
        mocker.patch.object(main.pwd_context, "hash", return_value="hashed")
        assert main.get_password_hash("pw") == "hashed"
...
class TestKosher:
    ...
    def test_alphanumeric_ok(self):
        assert main.kosher("alice123") is True

    def test_dash_ok(self):
        assert main.kosher("alice-bob") is True

    def test_empty_ok(self):
        """Empty string is technically kosher (the regex allows it)."""
        assert main.kosher("") is True

    def test_underscore_rejected(self):
        assert main.kosher("alice_bob") is False

    def test_space_rejected(self):
        assert main.kosher("alice bob") is False
...
class TestDecodeToken:
    ...
    def test_decode_without_verification(self):
        ...
        data = main.decode_token(token, private_key=False)
    ...
    def test_decode_with_private_key(self, valid_token, valid_token_payload):
        data = main.decode_token(valid_token, private_key=True)
    ...
    def test_decode_wrong_key_raises(self):
        ...
        with pytest.raises(Exception):
            main.decode_token(token, private_key=True)
...
class TestCanMint:
    ...
    def test_same_username_service_manager(self, service_manager_token):
        sub_data = main.decode_token(service_manager_token)
        mint_data = models.TokenData(
            username="testuser",
            site="myapp.example.com",
            target=settings.PROVIDER,
            provider=settings.PROVIDER,
        )
        assert main.can_mint(sub_data, mint_data) is True

    def test_same_username_same_site(self):
        sub = models.TokenData(
            username="u",
            site="same.com",
            target=settings.PROVIDER,
            provider=settings.PROVIDER,
        )
        mint = models.TokenData(
            username="u",
            site="same.com",
            target=settings.PROVIDER,
            provider=settings.PROVIDER,
        )
        assert main.can_mint(sub, mint) is True
    ...
    def test_different_username_raises(self):
        ...
        with pytest.raises(HTTPException):
            main.can_mint(sub, mint)

    def test_no_site_raises(self):
        sub_data = main.decode_token(
            jwt.encode(
...
class TestCertify:
    ...
    def test_valid_token_certifies(self, valid_token):
        assert main.certify(models.Token(token=valid_token)) is True

    def test_expired_token_fails(self, expired_token):
        with pytest.raises(HTTPException) as exc:
            main.certify(models.Token(token=expired_token))
        assert exc.value.detail == exceptions.TOKEN.detail

    def test_none_token_certifies_as_anon(self):
        """A None token should certify as anonymous."""
        assert main.certify(models.Token(token=None)) is True

    def test_bad_token_fails(self):
        with pytest.raises(HTTPException) as exc:
            main.certify(models.Token(token="not-a-jwt"))
        assert exc.value.detail == exceptions.TOKEN.detail
    ...
    def test_wrong_provider_fails(self):
        ...
        with pytest.raises(HTTPException):
            main.certify(models.Token(token=token))

    def test_anon_token_certifies(self, anon_token):
        assert main.certify(models.Token(token=anon_token)) is True
...
class TestIsPermitted:
    ...
    def test_none_token_is_anon(self, mock_db_with_term):
        ...
        result = main.is_permitted(
            models.Token(token=None), "owner", "myapi", "read"
        )
    ...
    def test_banned_user_denied(self, valid_token, mock_db_with_term):
        ...
        result = main.is_permitted(
            models.Token(token=banned_token), "owner", "myapi", "read"
        )
    ...
    def test_wrong_target_denied(self, mock_db_with_term):
        ...
        token = jwt.encode(payload, settings.PRIVATE_KEY, algorithm=settings.ALGORITHM)
        result = main.is_permitted(
            models.Token(token=token), "owner", "myapi", "read"
        )
        assert result is False
