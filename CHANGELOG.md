1.0.301 || 03.08.2026
docs: web10 v3 groups — policy containers, not data containers. Groups hold people, not posts. Two contract types: service contracts restrict (floor), group contracts grant (widen). Posts attach to groups to widen access. Groups carry across apps — one membership, infinite apps. Follows are groups. User panel has two views: service contracts and group contracts. ALSO rewrote v3 overview.md to align: removed `visibility` column and `collection_permissions` table, added `post_groups` table, replaced set intersection model with service+group contract model, replaced layered permissions with group membership checks, replaced follows table with groups-as-follows.
