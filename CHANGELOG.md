1.0.282 || 31.07.2026
fix(web10-social): D-public-media-verify — cross-user profile media renders (operator, 30.07 rant: "when i go to someones profile, i dont see their profile photos"). Root cause: resolveMediaRefs (posts.ts:350) accepted an \`owner\` parameter but never passed it to \`wapi.read()\`, so the wapi wrapper defaulted to the viewer's collection. The viewer's collection has no records for the author's media IDs → zero records → empty avatar, no post photos. One-line fix: pass \`owner?.username\` and \`owner?.provider\` to \`wapi.read()\`. 1 new vitest regression (cross-user owner params reach wapi.read). 445 tests green, tsc clean.
