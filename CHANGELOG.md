1.0.291 || 31.07.2026
fix(api): discovery board service allowlist — non-post services can no longer taint trending/recent/search/users. Root-caused from prod mongo (deploy DB): mad's 4 `fallout-avatar` records ghosted into the trending feed as empty posts because the discovery gate was "any anon-readable service" (`service_allows_anon` only), and fallout-avatar's terms whitelist `.*` read — so any app with an anon-read term leaked non-post records onto the board. Fix: new `DISCOVERY_BOARD_SERVICES = ("public_posts", "web10_apps")` allowlist in documentdb.py — `background_index_post` only indexes board services (write side), and every board read (`query_discovery_posts`, `search_discovery_posts`, `trending_topics`, `suggested_users`) filters `service $in` the allowlist via `_board_visible()` (read side, so already-indexed ghost docs stop showing immediately; the #web10apps app-store projection keeps working). 7 new regression tests (allowlist contents, anon non-board service not indexed, public_posts still indexed, service filter on feed/search/topics/users). 569 api tests green. Prod cleanup note: 4 stale `fallout-avatar` docs remain in `web10.discovery_posts` (now filtered from all board reads; purge optional). Separately diagnosed live: coolguydavid's missing post was an admin board-removal (`removed_by: jacoby149`, 27.07) — profile/friends feeds unaffected by design; restore via `/admin/discovery/restore`.
