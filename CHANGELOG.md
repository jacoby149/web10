...
1.0.279 || 30.07.2026
feat(api): A20-recovery-email-channel bite b — email password reset flow + real SMTP. Two new endpoints: `POST /email_recovery_prompt` (sends a 6-digit recovery code to the user's verified email; rate-limited 5/5min; rejects unverified emails and unknown addresses) and `POST /email_recovery_reset` (code + new password -> resets the password; rate-limited 3/10min; enforces 8-char minimum). Replaces the SMTP stub in `email.py` with real SendGrid v3 delivery (`_send_via_sendgrid` via urllib, keys from env only — `SENDGRID_API_KEY`, `SENDGRID_FROM_EMAIL` in `settings.py`); falls back to a print stub when the key is absent (safe for local dev and tests). Rate limiting uses a sliding-window in-memory `_RateLimiter` applied to both new email endpoints and the existing SMS `/recovery_prompt` + `/recovery_bot` endpoints. New exceptions: `RATE_LIMITED` (429), `EMAIL_NOT_VERIFIED` (400), `WEAK_PASSWORD` (400). 21 new tests (recovery prompt, reset, rate limits, SendGrid delivery + fallback), 575 total green, ruff clean.
