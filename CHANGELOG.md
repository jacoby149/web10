1.0.155 || 24.07.2026
D-follower-count: real follower count from the public ledger (D34). `followUser` mirrors to `/public/entries` with `payload.action='follow'` targeting the followed user — the D32 pattern reactions/comments already use. `unfollowUser` deletes the ledger entry so the count decrements. `countFollowers` reads from the ledger (O(1), no cross-collection reads). ProfileScreen gains the Followers tile with a real ledger-backed number (tabular-nums). The `Follow` schema is registered in `feed.ts` DEFAULT_SCHEMAS. The persona seed script backfills existing cross-follows into the ledger (idempotent). `deletePublicEntry` added to `feed.ts`. Decision D34 recorded in decisions.md. 62 tests green (210 pre-existing failures unchanged), tsc -b clean, build clean.
1.0.157 || 24.07.2026
