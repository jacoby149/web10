1.0.283 || 30.07.2026
feat(api): A21-passwordless-phone-login bite a — passwordless login via SMS code. Two new endpoints: `POST /passwordless/send` (triggers a Twilio Verify SMS; rate-limited 5/5min; same response shape for known/unknown phones — no user enumeration) and `POST /passwordless/verify` (Twilio Verify code check → mints a scoped token with `site="passwordless"`; rate-limited 3/10min; rejects unregistered phones). Code verification uses Twilio Verify (cryptographic check, satisfies I2). Rate limiting uses the shared in-memory `_RateLimiter` class (new `RATE_LIMITED` 429 exception). 12 new tests, 564 total green, ruff clean.
1.0.284 || 30.07.2026
