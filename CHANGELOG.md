...
1.0.289 || 30.07.2026
test(e2e): C8-stripe-twilio-test-keys-e2e bite c — Twilio honest half + Stripe webhook signature verification. Twilio smoke tests: direct SDK auth assertion, valid test number (+15005550006) verification succeeds, invalid test number (+15005550001) returns canceled, API /recovery_prompt integration test returns Twilio message SID against test creds. Header documents green != SMS delivered (manual pre-launch check). Stripe webhook signature tests: valid signature verifies, bad signature fails, tampered payload fails, missing signature fails. Twilio env vars wired through e2e compose + CI (TWILIO_TEST_ACCOUNT_SID, TWILIO_TEST_AUTH_TOKEN, TWILIO_TEST_SERVICE_SID, TWILIO_TEST_NUMBER — GitHub Actions secrets, zero in repo). GAP: API has no Stripe webhook endpoint; the signature mechanism is verified but un-testable against a real handler (.context/stripe-webhook-gap.md for lane A).
