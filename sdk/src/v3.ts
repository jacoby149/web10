...
export interface SessionVerdict {
  ...
  contract: { state: SessionContractState; missing_services: string[] }
  groups: { followers: SessionFollowersState }
...
export function createV3Client(options: V3ClientOptions = {}): V3Client {
  ...
  const client: V3Client = {
    ...
    /**
     * The confirmatory session-health check. The server runs the ACTUAL checks
     * (token decode, user lookup, app-contract check, followers-group
     * membership) and returns a typed verdict + ordered recovery `actions`.
     * The app's SessionGuard executes those actions — the client never guesses
     * from status codes. Read-only and idempotent; safe to call at mount and
     * after any failure.
     */
    async verifySession(options: VerifySessionOptions = {}): Promise<SessionVerdict> {
      ...
      return v3Post<SessionVerdict>('session/verify', body)
    },
