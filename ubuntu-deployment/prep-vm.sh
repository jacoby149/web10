...
info "  4. In Portainer: Stacks → Add stack → import docker-compose.staging.yml"
info "  5. In NPM: Proxy Hosts → add forward hosts for each subdomain"
info "     (target hostname = service name, port = 80, enable SSL + Let's Encrypt)"
info ""
info "Stack file: ubuntu-deployment/docker-compose.staging.yml"
