...
- [✓ 3.67.0] Video: hls.js player for video posts in the feed (Safari native fallback, vendored hls.js) — moved from the `hls` lane. The app is the D44 integration test end to end: upload writes the `video` minio leaf + queues the transcode (best-effort), the read path carries `transcoding_settings` + a fresh per-reader manifest sig, and the feed card + lightbox play adaptive HLS when done (direct read_url otherwise).
- [ ] Video: player chrome in the lightbox (quality dropdown via hls.js levels, speed 1x/1.5x/2x, fullscreen — the media demo's full player surface, video-experience.md player spec)
