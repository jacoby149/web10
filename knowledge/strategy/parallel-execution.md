...
- [✓ 3.15.0] `/stats` macro: node-wide `users_1d/30d/90d/1y` (all apps, same query minus `GROUP BY`); homepage stat bar leads with the all-time `users` count (3.27.4 — operator preference: all-time, not the 30d window)
