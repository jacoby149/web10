...
- [✓ 3.93.1] **The default (unedited) video upload skips the re-encode — the "Darth Vader" audio is gone** (operator: "for some reason with the videos, the audio sounds like darth vader or something in the video when watched, dont know if it is the hls, or the video player incorrectly reassembling the audio"). Follow-up to 3.88.0 (enforced default re-encode) + 3.90.1 (silent audio). **The root (a real-time-capture artifact in `editVideo`):** the `canvas.captureStream(30)` + `AudioContext` + `MediaRecorder` re-encode is a **real-time capture** — it plays the source and records each frame + the audio in wall-clock time. When the main thread stutters (frame draws, React work), the audio capture drops or mis-times samples, producing a pitch/timbre distortion (the "Darth Vader" effect). 3.88.0 made this hit *every* upload (the enforced default re-encode), so every unedited video carried the risk. The node's ffmpeg transcode + the hls.js player are both clean (verified: a 440Hz/120Hz tone survives the node pipeline at the correct pitch) — the distortion is baked into the webm at the client re-encode. **The fix:** the default edit (no trim, no crop) is a **no-op** — it re-encodes nothing, so the browser skips the lossy real-time path and uploads the original file directly; the node's ffmpeg transcodes it to clean HLS (the deterministic path). New `isNoopEdit(opts, duration)` helper (`src/lib/videoEditing.ts`); `PostComposer.handleSubmit` calls it and only re-encodes when the edit is real (a trim/crop from the editor). **Tests:** `videoEditing.test.ts` +8 (`isNoopEdit` state table); `videoEditor.test.tsx` re-pinned (an unedited video uploads the original directly, no re-encode; the no-op path shows no "Encoding" phase; the stale "always encodes" + "failed default encode" tests removed — the failed-*edit* case is covered by the editor tests). 71 touched/adjacent tests green, `tsc --noEmit` clean.
