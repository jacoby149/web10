...
- [✓ 3.73.0] **Always-on surface** (`src/components/Social/Layout.tsx`, `src/App.tsx`) — a bell with a live unread badge (desktop sidebar + mobile top-header) + an app-wide "N new" banner (clears on /notifications). Bell → `/notifications`. `useNotifications` hook + `NotificationBell`.
- [✓ 3.73.0] **The `/notifications` screen** (`src/components/Notifications/`, route in `App.tsx`) — history list (avatar + "did X" + time-ago), deep-linkable (`/notifications`), mark-read on open, designed empty state. (`?item=` deep-link + avatar resolution are follow-ups.)
