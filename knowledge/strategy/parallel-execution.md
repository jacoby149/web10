...
- [✓ 3.35.0] Groups: app-function filter + create group (operator: "the groups tab is showing groups that are clearly for private app functionality purposes … also no create group button … should be a create group button, that will then send a contract to the authenticator"): My Groups now drops the per-user app-function storage groups (`media-`/`notes-`/`sharing-{username}` — the app-backend groups the KB calls "infrastructure the apps create on the user's behalf") alongside the existing discover/followers/DM filter (`isAppFunctionGroup` in `src/data/groups.ts`); a **Create group** button on the My Groups tab opens a bottom sheet (name + join policy, all states) that sends a `create_group` GCR to the authenticator — `requestGroupCreation` (the IIFE SDK's `contractRequest`, the D42 consent pattern the demos run): the user approves there, the authenticator (the trusted party) creates the group with its own token, the sheet reports denied/error, approval reloads My Groups. 16 unit tests (9 data-layer: filter forks + GCR payload + no-SDK/no-token; 7 screen forks: button / dialog / empty-name validation / GCR sent / approved-reload / denied-stays-open / sdk-not-ready) + the `groups-create` screenshot-harness route. **Follow-up (social-e2e lane):** the browser gauntlet (create → real consent popup → approve → the group appears in My Groups).
