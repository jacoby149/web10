...
- [✓ 3.32.1] fix(marketing-ui): /trending consumes the v3 read shape — the 3.16.2 rewire's two regressions. (1) Negative times: `/v3/read`'s naive `str(datetime)` `created_at` was parsed as local time (west-of-UTC → future); new `parseCreatedAt()` normalizes to UTC, used by `timeAgo`/`commentTimeAgo`/`postToSignals` (clamped ≥0). (2) Empty Video view: the v3 read serves `media_refs` pre-resolved (objects with `mime_type` + `read_url`), but the page treated them as strings + hardcoded `first_attachment_mime: undefined`, so media was tag-detected only; now `first_attachment_mime` derives from the resolved ref and `TrendingMedia` uses `read_url` directly (no token-gated presign round-trip). 6 new tests, 241 green. **Follow-up (root cause, not this bite):** the naive `created_at` is an API serialization issue — the social app's `formatTimeAgo` has the same latent shift; fixing it at the API (ISO 8601 UTC) needs live-CH verification first.
