...
- [✓ 3.83.0] The feed + discover rack is just Recency / Likes / Comments — the Time knob is gone, the half-life is fixed at 1 day (operator: "the tune your own feed and discover should be just recency likes, and comments! the time knob is too complex… we should just set that to something sensible for the user!"). The **Time knob (the recency half-life) is removed** — the same move as the Character knob (3.71.0): the obfuscated math dial (1h / 4h / 12h / 1d / 7d / ∞) with no plain-English concept. The half-life is **fixed at the middle detent — 1 day** (`FIXED_HALF_LIFE_DETEENT` / `FIXED_HALF_LIFE_MS` in `powerMean.ts`); `halfLife` stays in `KnobState` so `?knobs=` + saved settings keep their 5-field shape, but `scorePost` + `knobStateToSort` pin it to the fixed value (the `FIXED_CHARACTER_P` pattern). The rack is now the three signals a user understands (no mobile sideways scroll). "Most loved · all time" is unaffected (all-time = recency weight 0, not the half-life). **The node contract is unchanged** (`PowerMeanSort.half_life_ms` still sent, now always 1 day; the server's `_power_mean_score` / `_power_mean_score_sql` untouched). **Cross-lane touch:** the verbatim copies `marketing/marketing-ui/src/lib/powerMean.ts` + `src/components/KnobRack.tsx` (+ `powerMean.test.ts` + `Trending.test.tsx` re-pinned to 3 knobs, `knob-time` absent) kept in sync. `powerMean.test.ts` (the fixed-half-life pins) + `feedKnobs.test.tsx` +1 (the rack is 3 knobs, `knob-time` absent). 497 social + 260 marketing-ui tests green, `tsc` clean. D36 amendment.
- [✓ 3.83.1] Clicking "Newest" on the feed now lights up — an explicit preset click beats the saved tuning (operator: "when i click newest button, it doesnt light up like the other two"). Feed-only precedence bug exposed by 3.83.0: `setKnobUrl` omits `?knobs=` for the feed default (Newest), so `knobState` (URL > saved > default) fell back to the saved tuning and the chip showed the saved preset, not Newest. Fix (`FeedScreen.tsx`): an explicit click that IS the feed default clears the saved tuning (`savedKnobsRef` → null) so the click wins; non-default presets untouched. `feedKnobs.test.tsx` +1. Discover + the other two presets were already correct.
