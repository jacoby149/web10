...
**Owns:** `marketing/web10-social/public/serviceWorker.js`, `marketing/web10-social/registerWorker.js`, `marketing/web10-social/src/components/` (the `InstallPrompt` surface + `useInstallPrompt` hook), `marketing/web10-social/src/lib/analytics.ts` (the install events), `e2e/tests/social-pwa.spec.ts`; cross-lane: `knowledge/knowledge-base/web10-v3/social/pwa.md` (the spec).
...
- [ ] **KB** (`knowledge/knowledge-base/web10-v3/social/pwa.md`) — the installability bar, the SW design (shell-only precache + offline fallback + update flow; the line: the cache is the app shell, never user content, the feed is always re-read from the node), the `beforeinstallprompt` → custom-surface wiring, the trigger policy (Shorts-on-phone / post-engagement, one dismissible surface, never a modal wall, never on a timer), and the D56 line (the "installed" event is a content-free GA4 action). **Gates the rest.**
- [ ] **The service worker (keystone)** (`public/serviceWorker.js` + `registerWorker.js`) — replace the stub (`const assets = []`) with a real app-shell precache (built JS/CSS/HTML + manifest icons), an offline fallback for the SPA `index.html` route (deep links like `/shorts` / `/u/:username` load the shell offline), and an update flow (versioned cache name + `skipWaiting`/`clients.claim`). The shell-only cache is the invariant. **Gates the prompt + e2e bites.**
- [ ] **The install prompt surface** (`src/components/` — `InstallPrompt` + a `useInstallPrompt` hook wrapping `beforeinstallprompt`) — capture the event, expose `prompt()`, render one tasteful dismissible surface (mobile bottom sheet / desktop banner, the `AdPicker` idiom) — "install to keep your feed + go offline." Dismissal remembered in `localStorage` so it never re-nags the session.
- [ ] **The trigger policy** (wired into `ShortsScreen.tsx` + the engagement path) — show the surface when (a) a phone user opens `/shorts`, or (b) a user likes/follows a creator. Never on page load, never on a timer, never past the one remembered dismissal. Desktop: the explicit "Install app" affordance only (the browser's own prompt is the desktop path).
- [ ] **Telemetry (the D56 line)** (`src/lib/analytics.ts`) — content-free GA4 `pwa_install_prompt_shown` + `pwa_installed` actions (a count + the trigger context: `shorts` / `engagement` / `manual`). No user content, no PII.
- [ ] **Unit tests** — `useInstallPrompt` (event captured, `prompt()` called, dismissal remembered), `InstallPrompt` (renders on trigger, dismisses, no re-show after dismissal, mobile sheet vs desktop banner), the trigger policy (Shorts-on-phone ✓, like/follow ✓, page load ✗), the SW (shell precache list, offline fallback resolves the SPA route, versioned-cache update path). Screenshot harness: the install surface on a 375px Shorts view + a desktop banner.
