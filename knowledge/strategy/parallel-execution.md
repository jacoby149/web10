...
- [✓ 3.82.1] Like button is a toggle, not a counter (operator: "i liked a post three times, should just toggle like unlike"). Rapid taps fired concurrent `toggleReaction` calls — each read before any create landed, all created → the same user's like stored N times (the node counts docs, not distinct users, so the count inflated). Fix: (1) in-flight guard on both like surfaces (`FeedScreen` per-post `pendingLikes` set, `PostLightbox` `likeInFlight` ref — a tap while in flight is a no-op, released in `finally`); (2) self-healing `toggleReaction` (deletes ALL of the caller's matching reactions, both signature paths — pre-existing duplicates heal on the next unlike+re-like). `reactions.test.ts` re-pinned to drive the real `toggleReaction` (the old toggle tests were ghost tests) + self-heal case; feed + lightbox guard tests. 498 social tests green, `tsc` clean.
