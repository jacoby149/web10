...
- [✓ 3.31.0] Groups screen: the "coming soon" Groups tab is now real — My Groups (community memberships, infra groups filtered) + Discover (the D53 directory, search + topic filter) + the deep-linkable detail (`/groups/:id`). Data layer: `readGroupDirectory` + `readGroupDetail` (the public GET endpoints) + the community-group filter. 22 unit tests + screenshot harness routes.
