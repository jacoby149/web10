...
- [ ] Video: **cross-user HLS streaming** — a FOLLOWER's minted sig is 403 at the manifest fetch: `can_view_doc` = author or member of the MEDIA doc's groups, and social media docs (confirmMediaUpload) are groupless, so only the author's own transcoded posts stream (3.67.0 proved the author path end-to-end; a follower sees the player's designed error state). The KB (`media/minio-auth-bifurcated.md`) assumes the feed-viewer re-check works — the access model must be decided: (a) attach media docs to their post's groups at post time (the D58 write gate + the discover-group role shape need checking), or (b) extend `can_view_doc` to "reader can read the post that carries this media" (a reverse-lookup at manifest time — heavier, and a security-model change → decisions.md entry). Gated on the decision; the player + data layer are done.
