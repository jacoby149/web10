...
- [ ] **The missing primitive — `getPendingRequests`** (`api/` + `sdk/`) — a read of the followers group's pending join requests (`requestJoin` exists, the read does not). **Gates** follow-request notifications.
...
- [ ] **Follow-up** — group permalink (same preview-server pattern: the group's face — cover + avatar from the `web10-social-group-identity` doc, D60 — as the image). A new route in the same social preview server.
