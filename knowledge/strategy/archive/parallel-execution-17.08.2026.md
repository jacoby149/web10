...
- [✓ 3.0.36] Popup-blocked error fix — SDK contractRequest reuses open auth popup to avoid browser popup blocker. UI ACRListen uses wildcard origin for cross-origin popup messaging. (#592)
- [✓ 3.0.38] _popupReady check + sdk/demos/ cleanup — second contractRequest (group) sends immediately if popup already sent auth_ready, fixing notes demo where group contract was never delivered. Removed sdk/demos/ duplicates — demos live in marketing/marketing-ui/public/docs/ only. Marketing-ui Dockerfile copies sdk/dist/browser.js → wapi.js at build.
