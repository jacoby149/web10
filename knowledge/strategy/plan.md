...
- [ ] **UI: the directory screen** (`marketing/marketing-ui/` or `web10-social/`) — the browse surface: minimal grid (apps enrich) + deep-linkable detail (with the "join to view posts" state); topic search by filtering the directory's tags client-side.
