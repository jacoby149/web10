...
- [✓ 3.36.0] **Groups: app-function filter + create group** (`src/data/groups.ts`, `src/components/Groups/`) — My Groups now drops the per-user app-function storage groups (`media-`/`notes-`/`sharing-{username}` — the app-backend groups the KB calls "infrastructure the apps create on the user's behalf") alongside the existing discover/followers/DM filter, and a **Create group** button (My Groups tab) opens a bottom sheet (name + join policy) that sends a `create_group` GCR to the authenticator — the D42 consent pattern the demos run: the user approves there, the authenticator (the trusted party) creates the group with its own token.
