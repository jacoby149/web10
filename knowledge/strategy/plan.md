...
- [ ] **API: `GET /v3/groups/directory`** (`api/app/v3/endpoints/groups.py`, `services/clickhouse.py`) — public, anon, paginated; the **minimal** list of `discoverable = true` groups: id, name (identity record, else slug), owner, join policy, member count, permission summary. **No tags, no posts.** A view over `group_contracts` ⋈ `group_members` ⋈ `group-identity-service`.
- [ ] **`group-identity-service` read path** (`api/app/v3/services/clickhouse.py`) — read a group's identity record (name, description, banner, avatar, website, tags) from the documents collection (append-only, latest wins); the source for the directory name + the detail display + app enrichment. (Write path is the owner/`page-curator`'s job — a normal document create.)
- [ ] **API: the group detail (by ID)** — public, principal-based (token optional, `user_or_anon`): metadata (contract, member count, identity) always for an existing group; posts only if the *reader* is a member (else "join to view"); **only a non-existent group 404s** (unlisted-model — a non-discoverable group does not 404).
...
- [ ] **UI: the directory screen** (`marketing/marketing-ui/` or `web10-social/`) — the browse surface: minimal grid (apps enrich) + deep-linkable detail (with the "join to view posts" state); topic search by composing the directory with an identity query.
- [ ] **Tests** — unit (directory = the `discoverable` groups, minimal fields only; a non-discoverable group is absent from the list; the detail returns a non-discoverable group — no 404; detail posts gated on the *reader's* membership: member sees posts, non-member gets "join to view", anon sees posts only if `anon` is a member; the discover group is anon-readable but NOT in the list; identity record read + slug fallback) + e2e (list → detail → anon reads posts on a public group; "join to view" on a listed-but-private group; a signed-in member reads a non-discoverable group's detail).
