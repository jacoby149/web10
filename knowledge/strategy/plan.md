...
- [ ] **Follow-up: group permalink** — the same endpoint pattern for `/groups/:id` (the group's face — cover + avatar from the `web10-social-group-identity` doc, D60 — as the image). The post + profile permalinks ship first (the ones the Share button / profile page emit).
