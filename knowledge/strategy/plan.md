...
- [✓ 3.67.0] **HLS in the feed** (`src/lib/useHlsVideo.ts`, `src/components/Feed/FeedScreen.tsx`, `src/components/Bio/PostLightbox.tsx`) — adopt the media demo's hls.js player (Safari native fallback, vendored hls.js) for video posts. The app is now the D44 integration test end to end: `uploadMedia` writes the `video` minio leaf + queues the transcode (best-effort), the read path carries `transcoding_settings` + a fresh per-reader manifest sig (cross-lane touch, the 3.34.0 precedent), and the feed card + lightbox play adaptive HLS when the transcode is done (direct read_url otherwise). Player chrome (quality/speed/fullscreen) is a follow-up lane item.
