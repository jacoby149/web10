...
- [✓] **KB: `node-ads.md`** (`knowledge/knowledge-base/web10-v3/social/node-ads.md`) — the node ad object, the read-time attachment (the third join), the density control, the renderer (both ads on the same post), the operator's revenue model (hosting + node ad revenue 85-90%), the "what this is NOT" (not a payment processor, v3 is ads only), security invariants. (3.62.0: the prose drift fixed — the node ad attaches to *any* post at the percentage, both ads coexist, not "only fills the `ad_mode = 'none'` gap.")
...
- [✓ 3.62.0] **Ad Inventory card (authenticator)** (`ui/src/components/Studio/`) — percentage slider (0-100, writes `node_ad_percentage`), list of active node ads (creative, offer, status), create/pause/resume/retire (writes `posts` docs tagged `ad` + `node_ad` to the discover group); all states designed (empty → CTA, skeleton, error)
- [✓ 3.62.0] **Tests** — unit (`api/tests/test_node_ads.py`: query + attachment, percentage 0/10/100, determinism per (doc, reader), round-robin, the third join, I3) + e2e (`e2e/tests/node-ads.spec.ts`: operator creates node ad → feed shows it → pinned post shows BOTH ads → percentage 0 = off; browser gauntlet: Ad Inventory card → "Sponsored" block → pinned post shows both)
