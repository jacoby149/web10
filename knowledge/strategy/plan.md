...
- [✓ 3.74.0] **The `/notifications` screen** (`src/components/Notifications/`, route in `App.tsx`) — the history list (avatar + "did X" + time-ago, the KB's screen shape), deep-linkable (`/notifications`), mark-read on open, designed empty state. (`?item=` deep-link + avatar resolution are follow-ups.)
