...
- [✓ 3.73.0] **The always-on surface** (`src/components/Social/Layout.tsx`, `src/App.tsx`) — a bell with a live unread badge (desktop sidebar + mobile top-header, where New-post/Log-out already live) + an app-wide "N new" banner (clears on /notifications) that surfaces without a dedicated screen. The bell → `/notifications`. `useNotifications` hook + `NotificationBell`.
- [✓ 3.73.0] **The `/notifications` screen** (`src/components/Notifications/`, route in `App.tsx`) — the history list (avatar + "did X" + time-ago, the KB's screen shape), deep-linkable (`/notifications`), mark-read on open, designed empty state. (`?item=` deep-link + avatar resolution are follow-ups.)
