...
Accounts, content, groups, access control, monetization, and moderation.
...
**18 tables.** Accounts, content, groups, access control, monetization, moderation.

## Provider Schema

Platform-level: app store, reviews, moderation, origin blacklist.
...
**4 tables.** App store, reviews, moderation, origin blacklist.

## Ad Schema

Programmatic advertising: campaigns, targeting, creative, inventory, tracking, partners, revenue.
...
erDiagram
    ad_campaigns {
        ...
        UInt64 impression_count
        UInt64 click_count
        UInt64 conversion_count
        UInt64 spend_cents
        DateTime64 start_date
        DateTime64 end_date
        DateTime64 created_at
...
**Counters are materialized.** `impression_count`, `click_count`, `conversion_count`, and `spend_cents` are incremented on each ad event. `ReplacingMergeTree` keeps the latest. Read is O(1). The `ad_impressions` / `ad_clicks` / `ad_conversions` tables are the append-only audit trail (TTL 30 days). You get fast reads for billing and full history for analytics.

**Provider campaigns.** `provider_key` allows the node operator to run their own campaigns (cross-promoting creators, platform promos, featured content). A campaign has either `advertiser_key` (user) or `provider_key` (platform) — not both.
...
**Primary key:** `campaign_id`. Status updates and counter increments are new inserts with higher `updated_at`. Budgets are enforced at serve time — if daily or total budget is exhausted, the campaign stops serving.
...
CREATE TABLE ad_targeting (
    ...
    targeting_value String,    -- JSON: { age_range, gender } or { interests: [...] } or { group_id } or { doc_id }
    created_at DateTime64(3)
) ENGINE = MergeTree()
ORDER BY (campaign_id, targeting_type);
```

**Primary key:** `(campaign_id, targeting_type)`. Multiple targeting rows per campaign — all must match (AND logic) or any can match (OR logic, controlled by the campaign). `targeting_value` is JSON for flexibility: `{ "age_min": 18, "age_max": 35 }`, `{ "interests": ["jazz", "music"] }`, `{ "group_id": "web10.app/groups/alice/followers" }`, `{ "doc_id": "post-123" }`.
...
**Per-post targeting.** `targeting_type: 'doc_id'` lets an ad target a specific post (sponsored post placement). The ad appears alongside or within that post's content.
...
Where ads can appear. Feed slots, sidebar slots, between posts, story ads, per-post placements.
...
**Primary key:** `slot_id`. Defines where and how ads can appear. `audience_scope` controls targeting depth: `public` (anyone), `group` (group members only), `personalized` (user-specific based on behavior). `placement: 'in_post'` is for per-post ad placements (sponsored content within a specific post).

### Ad Impressions

When an ad is shown to a user. Immutable audit trail. TTL 30 days.
...
**Primary key:** `impression_id`. Immutable — no tombstones, no updates. High volume: millions per day. TTL removes old data after 30 days. `creator_key` is the creator whose audience was served (e.g., the ad appeared in a follower's feed while they were viewing the creator's content). Revenue splits: creator gets a share of `revenue_cents`, platform keeps the rest.

**Counters on `ad_campaigns`** are incremented on each impression (append-only, materialized). The impressions table is the audit trail (analytics, fraud detection, attribution). You get both: fast reads for billing, full history for analytics.

### Ad Clicks

When a user clicks an ad. Immutable audit trail. TTL 30 days.
...
**Primary key:** `click_id`. Immutable. Links back to the impression that generated the click. Used for CPC billing and CTR calculation. Counter on `ad_campaigns` incremented on each click.

### Ad Conversions

When a click leads to a desired action (purchase, sign-up, etc.). Immutable audit trail. TTL 90 days (longer retention for attribution windows).
...
**Primary key:** `conversion_id`. Immutable. Links back to the click that led to the conversion. Used for CPA billing and ROAS calculation. Counter on `ad_campaigns` incremented on each conversion.
...
| **Counters** | Materialized on the entity row, incremented via `ReplacingMergeTree` | Fast reads (O(1)). Audit trail in separate event table with TTL. |
...
| **TTL** | `TTL created_at + INTERVAL N DAY` | Physical cleanup. Old data disappears automatically. |
