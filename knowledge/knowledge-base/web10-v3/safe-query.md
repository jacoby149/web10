...
## Wiring it in (not done yet)

`build_safe_query` is the boundary. To use it, the read endpoint would:
1. take the caller's `SELECT` + the app-contract-granted services;
2. compute `readable_groups_by_service` via the D58 read gate
   (`readable_groups()` per service);
3. call `build_safe_query(...)`;
4. run the result.

Until wired, the existing fixed read shapes (group read, `get_ref_counts`)
remain the path — see `query-engine.md` phasing.
