...
Documents are opaque JSON. The leaf-level type convention lets the API do useful things (convert MinIO URLs, sort numbers, filter booleans) without knowing the schema. The app owns the structure. The API trusts the types. Weak typing. Freeform but useful.