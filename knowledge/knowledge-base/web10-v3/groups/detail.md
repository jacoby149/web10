...
| Can a reader see its **posts**? | membership (I3) | the real permission boundary — independent of `discoverable` |

`discoverable` is a **blasting** flag, not a **permission** flag. It controls whether the group is *advertised* in the directory. It never gates the detail, and it never gates content. The permission boundary is membership, full stop.
...
- Its **posts** are still governed by membership (I3) — the reader sees them only if they're a member.

So `discoverable` answers "is it blasted?" and membership answers "can this reader see the content?" A non-discoverable group is one you can find *if someone gives you the ID*, but you still have to be a member to read the posts.

**Only a non-existent group 404s.** `detail?id=` on a group ID that doesn't exist returns 404. A group that exists but is `discoverable = false` does **not** 404 — it returns normally (metadata always, posts per membership).
...
Everything the reader sees is then governed by *that principal's* membership in the group. This is what makes it flexible: the same endpoint serves anon and any signed-in user, and each sees exactly what their own membership allows. An app holding a member's token reads the group as that member; an app with no token reads it as `anon`.
...
- The reader is **not a member** → no posts; the response carries a "join to view posts" state (the group is findable, its content is gated). This is the flexible "here's a group, join it" state — not a 404.

Because the read is principal-based, "member" and "not a member" are evaluated against *the reader*, not against `anon` specifically. Anon is just the principal when there's no token.
...
A directory entry deep-links to the detail. An anon user clicking a directory entry reads the detail as `anon` (metadata + posts if `anon` is a member). A signed-in user reads it as themselves (metadata + posts if they're a member). Same endpoint, different principal, different result.

## Security Invariants

- **I3 holds.** Posts are returned only when the *reader* is a member with `readAll` — the same membership gate that enforces I3 for every other read. `discoverable` plays no part in content access.
- **Metadata is low-sensitivity.** The contract (join policy, roles, member count) is readable by ID. This is accepted: group IDs are guessable URLs anyway, and the real boundary is the content, which stays membership-gated. (A DM group's *metadata* — "invite_only, 2 members" — is visible by ID; its *posts* are not.)
