...
**The hover preview (the `HomeCard`'s 16:9 tile, 3.164.0; the no-gray + progress bar + audible-unmute pass, 3.165.1).** The Home wall's tile is the one surface that plays on *hover*, not tap — the YouTube home behavior (the operator, 25.09.2026: "when you hover on youtube, the video starts playing … even has that nice audio icon in the top right to toggle volume"). The shared `HoverVideo` (`@web10/discover`) renders the tile's poster at rest; when the pointer **enters** (a hover-capable device — touch never fires `mouseenter`, so a phone keeps the static thumbnail + the tap navigates) the video starts playing **muted** (the autoplay policy), `object-cover` in the same 16:9 frame, and a **speaker icon (top-right)** toggles the sound (the user's explicit gesture, so un-muted playback is allowed). A thin **progress bar** tracks the preview's position along the bottom of the tile (the YouTube home behavior — the operator, 25.09.2026: "on youtube there is a progress bar in the little thing"). When the pointer **leaves**, playback stops and the poster returns. The source follows the same rule as every other surface (`sourceFromMedia`: transcoded → hls.js, else the direct file) and attaches **lazily on first hover** — a wall of cards does not mint/attach N players at rest. The frame is **inert to the pointer**: the card's `<a>` (the post's permalink) owns the click — hovering plays, clicking navigates. A failed attach (an undecodable codec, an expired sig) degrades to the poster; the tile still works as a link. The play affordance + the duration badge + the progress bar are `pointer-events-none` (decorative) so they never sit over the `HoverVideo` and steal its `mouseenter`.

**The reveal is keyed to `playing`, not `canplay` (the no-gray fix, 3.165.1).** The video fades in only once it is actually **playing** (frames on screen) — `canplay` fires *before* the first frame renders (notably for HLS), so keying the crossfade to it left a gray/black tile (the operator, 25.09.2026: "the vid goes gray"). The poster stays as the backdrop the whole time, so the tile never flashes: at rest and while the source loads the poster is the visible face, and the video crossfades in over it the moment frames are on screen. Un-muting is **audible** (the operator: "no volume") — toggling the speaker raises `volume` to `0.8` if it was at zero, so the un-muted preview is actually heard, not just un-muted at zero volume.
