...
**Playback (the ambient loop):** the **active slide autoplays** (muted — the browser's autoplay policy + the feed's ambient idiom); **off-screen slides pause** (the IntersectionObserver the deep-link sync already uses is the source of truth for "active"; a slide that drops below the ~60% threshold pauses its video and resets to the poster). Tap the video = play/pause in place (the `InlineVideo` invariant: the tap never escapes the slide). **Muted by default, with an escape hatch (decision #7):** the lens opens muted (autoplay policy), but the user must be able to *hear* the video — the action rail carries a speaker icon (top item, above the heart — the TikTok position) that toggles a **screen-level** mute state: one tap un-mutes every short for the session (the choice is a module-level flag — it survives leaving + returning to Shorts within the session; a page reload is a fresh lens, back to muted), the icon flips `VolumeX` ↔ `Volume2`, and the tap `stopPropagation`s (a sound tap never toggles play/pause). The player side is the `muted` seam on `<VideoPlayer immersive>` (default `true`; the surface passes its state) — the `<video>` follows the prop on mount + on every flip. The operator's "keep sound" rule (3.90.1) is about *uploads not shipping silent files*, not about this surface playing with sound by default.
...
**The mobile lens is full-bleed — the bottom tab bar is hidden on it (decision #8):** on a phone the lens is an immersive surface, and the app's mobile bottom tab bar (Feed/Discover/Messages/Profile/More, `fixed bottom-0 z-20`) would paint over the action rail (like/comment/share, `absolute bottom-6 right-3`) and the comment sheet (`absolute inset-x-0 bottom-0 z-10`) — both anchored to the slide's bottom edge, which on a real device (mobile browser chrome, `100vh` vs the visual viewport) lands inside the nav band. The fix is the TikTok model, not a toggle: `Layout` hides the bottom bar when `pathname.startsWith('/shorts')` **and** drops the wrapper's `pb-16` reserve, so the video runs edge-to-edge (the lens is truly full-bleed, no dead band under the bar). The mobile top header stays (account actions remain reachable). **The exit is a back arrow the lens renders itself** — top-left over the video (`sticky top-4 left-4 z-30`, a `ChevronLeft` pill that stays pinned while you swipe), `aria-label="Back to feed"`, → `navigate('/feed')` (the home base; the operator's call — feed, not discover). No manual show/hide toggle: the bar is simply absent on the lens, and you leave by the arrow (or the system back gesture). **The `slideEls()` invariant:** the back arrow is a child of the swipe container, so anything that addresses a slide by position must filter to the slides, not index `container.children` raw (the button would shift every index by one and break the keyboard swipe + the active-slide observer + the deep-link scroll). `ShortsScreen`'s `slideEls()` returns the container's `[data-testid^="short-slide-"]` children in order, and the keyboard handler, the `IntersectionObserver`, and the deep-link scroll all use it.
...
7. **The sound toggle** (operator, 17.09.2026: "tik tok view looks great but muted default, should he a mute unmute speaker icon, no way to hear the video") — muted-by-default stays (the autoplay policy), but the lens gets the TikTok escape hatch: a speaker icon on the action rail (top item, above the heart) toggles a **screen-level** mute state (one tap un-mutes every short for the session — a module-level flag, not per-video, not persisted past the session); the `muted` seam on `<VideoPlayer immersive>` (default `true`) is the player side.
8. **The mobile lens is full-bleed — the bottom tab bar is hidden on it, and a back arrow is the exit** (operator, 18.09.2026: "on mobile, the bottom bar is blocking both adding a comment and share button … when you open comments it should hide the shorts bar always" + "an arrow that gets you out … brings you to feed"). The TikTok model, not a show/hide toggle: `Layout` hides the mobile bottom bar on `/shorts*` and drops the wrapper's `pb-16` (the video runs edge-to-edge); the lens renders a back arrow top-left over the video (`sticky top-4 left-4 z-30`, `ChevronLeft`, `aria-label="Back to feed"`) → `navigate('/feed')` (feed, not discover — the operator's call). The back arrow is a child of the swipe container, so slides are addressed by `slideEls()` (the `[data-testid^="short-slide-"]` children), never by raw `container.children` position (the button would shift every index by one and break the keyboard swipe + the active-slide observer + the deep-link scroll).
