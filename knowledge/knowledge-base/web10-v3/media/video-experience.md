...
**The default edit is a no-op fast path.** "Original" (source ratio, full duration) is the default preset. When the user posts a video they never ran through the editor, that default edit is a **no-op** — no trim, no crop — so the browser skips the re-encode entirely and uploads the original file directly; the node's ffmpeg transcodes it to HLS (the deterministic path). The re-encode (`canvas.captureStream` + `MediaRecorder`) is a **real-time capture**: it plays the source and records each frame + the audio in wall-clock time, and when the main thread stutters the audio capture drops or mis-times samples, producing a pitch/timbre distortion (the "Darth Vader" effect). A no-op edit re-encodes nothing, so running it through that lossy path was pure risk with zero benefit. **Only a real edit (trim / crop, from the editor) runs the re-encode** — that is when the browser must produce the finished file. Both paths show progress (the re-encode is real-time — as long as the clip — and the upload is a byte-count), so the user is never left in the dark.

**The re-encode preserves the source audio.** The finished file carries the same sound as the original — a silent webm is a bug, not a feature. The client routes the source `<video>`'s audio through an `AudioContext` into the `MediaRecorder` stream; the element is **not** muted (a muted element silences its `MediaElementAudioSourceNode` in Chrome/WebKit and ships a silent file), and the `AudioContext` is `resume()`d before recording (it can boot suspended without a fresh user gesture). Muting the element is only a fallback for the case where audio routing genuinely fails.
