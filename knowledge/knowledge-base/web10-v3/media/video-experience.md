...
- Trim / reframe / ratio-pick = browser work (ffmpeg.wasm — `client-side-transcoding.md`)
...
**The default edit is a no-op fast path.** "Original" (source ratio, full duration) is the default preset. When the user posts a video they never ran through the editor, that default edit is a **no-op** — no trim, no crop — so the browser skips the re-encode entirely and uploads the original file directly; the node's ffmpeg transcodes it to HLS (the deterministic path). This also keeps the common (unedited) upload from ever paying for the ~32MB client engine. **Only a real edit (trim / crop, from the editor) runs the re-encode** — that is when the browser must produce the finished file.

**The re-encode is ffmpeg.wasm, not a real-time capture.** The edit path runs the full FFmpeg binary in a Web Worker as a **deterministic file-to-file op** (`src/lib/ffmpegEngine.ts`): no playback, no `canvas.captureStream` + `MediaRecorder` real-time capture. This kills two bugs the real-time path had — (1) the audio capture dropped/mis-timed samples when the main thread stuttered (the "Darth Vader" pitch distortion), and (2) the audio ran on a clean Web Audio clock while the video was captured via rAF, so dropped frames drifted the A/V. The ffmpeg trim is frame-accurate (`-ss` before `-i` + re-encode), the ratio reframe is a `crop` filter, and the audio is `-c:a aac -ar 48000` (clean, standard sample rate) — guaranteed A/V sync. The ~32MB core is **self-hosted** (Vite emits it as an asset from our origin, not unpkg) and **lazy-loaded on editor open** (the intent signal — it downloads in the background while the user adjusts trim/crop, cached afterward). The single-threaded core needs no COOP/COEP headers. The node still re-transcodes to HLS, so the client's output just needs to be valid + trimmed + cropped — the preset is `veryfast` (fast, good enough for social; the node's ffmpeg is the final-quality path).
