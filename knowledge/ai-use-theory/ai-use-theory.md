...
- [A Real-World Example](./real-world-example.md) — the theory in action, one real session end to end (a consent popup glitchy for two weeks): the root cause (one popup doing two jobs), the fix, reds triaged, a real bug found in the local logs, the environment regressed and fixed locally, suite green then CI-confirmed
