...
/**
 * The shared multi-media carousel (video-player.md) — a fixed frame + a
 * scroll-snap strip of all the post's media, with a position indicator (1/N).
 * Video slides render through `<VideoPlayer fill>` (tap-to-play); image slides
 * are `<img>`. Both the feed and discover compose it, so the "no surface owns
 * a `<video>`" rule holds. The frame ratio is explicit (discover = 16:9) or
 * the first item's natural ratio (feed), so swiping never shifts the layout.
 */
